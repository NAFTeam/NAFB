...
class Logging(Extension):
    ...
    @staticmethod
    def is_naff_server(event: BaseEvent):
        g_id = 870046872864165888
        if isinstance(event, GuildEvent):
            return event.guild_id == g_id
        if isinstance(event, MessageDelete):
            return event.message.guild and event.message.guild.id == g_id
        if isinstance(event, MessageUpdate):
            return event.before.guild and event.before.guild.id == g_id

    @listen()
    async def on_member_update(self, event: MemberUpdate):
        if self.is_naff_server(event):
            before = event.before
            after = event.after
    ...
    async def on_member_add(self, event: MemberAdd):
        if self.is_naff_server(event):
            ...
            emb.set_author(
                name=event.member.tag, icon_url=event.member.display_avatar.url
            )
    ...
    async def on_message_delete(self, event: MessageDelete):
        if self.is_naff_server(event):
            ...
            if (
                ...
                and (count := len(event.message.attachments)) > 0
            ):
